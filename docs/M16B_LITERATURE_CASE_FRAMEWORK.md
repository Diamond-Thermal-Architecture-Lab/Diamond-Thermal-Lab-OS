...
The [schema proposal](M16B_LECF_SCHEMA_PROPOSAL.md) defines six logical records: Literature Metadata, Evidence Mapping, Engineering Parameter Extraction, Case Reconstruction, Validation Status, and Gold Candidate Status. It proposes identifiers and revision bindings without modifying current schemas.
...
3. **Reconstruct before evaluating.** Bind geometry, material conditions, thermal and mechanical boundaries, process sequence, observables and comparator conditions. Keep original observations separate from reconstructed assumptions and counterfactual variants. Review the exact map into EPR inputs.
4. **Predeclare the comparison.** A validation protocol records target quantities/locations, input and target dataset partitions, model-selection/fitting history, residual definitions, acceptance rationale, uncertainty treatment, invalid-run rules, controls and exclusions. Numerical tolerances require a case-specific evidence basis and independent review; this proposal supplies none. Freeze the protocol, reconstruction, EPR, intended Plan content identity and input visibility before evaluation/reveal. The protocol references the Plan hash; it does not create a third primary M16A Plan artifact.
5. **Execute and record.** The existing kernel executes only an applicable Plan; EER embeds that exact Plan/Manifest. Capture all planned scenarios and non-evaluable results. Project only eligible prediction outputs. A mismatch, null prediction or unsupported observable remains an informative finding.
6. **Assess independently.** Compare with eligible evidence, record residuals and possible parameter/model/boundary/measurement causes without attributing cause from residual alone. Independent reviewer checks scope, lineage, exposure and all adverse findings before considering candidacy. Rule/model changes require a separate work item and fresh validation.

Data lineage uses experiment/dataset families, sample groups and reused-data relations across papers, not DOI uniqueness. Each dataset records roles such as input, calibration, model selection, validation target or control. If a target influenced TBR fitting, property choice, boundary selection, model choice, thresholds or sensitivity settings, its comparison is reconstruction/calibration evidence and cannot also count as independent validation. An unknown relationship stays unresolved. Merely splitting figures, citations, samples or files does not prove independence; shared fits, apparatus/systematic errors and investigator exposure need assessment.
...
Required review outputs are a search/screening register, source-version and dataset-lineage register, assertion/parameter sheets with independent check differences, reconstruction and gap sheets, a predeclared validation protocol with controls, and a pilot assessment. For a genuinely compatible case, use unchanged EPR/Plan/EER/projection contracts; otherwise produce a specific incompatibility report. Include a draft prior-art brief, neutral comparison, failure/negative-evidence retrieval example, validation plan and internal customer-memo draft. Every statement in that memo must map to a source or be explicitly conditional; it is not released by the pilot.
