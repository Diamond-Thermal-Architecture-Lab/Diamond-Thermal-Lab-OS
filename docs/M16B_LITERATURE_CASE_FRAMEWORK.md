...
3. **Reconstruct before evaluating.** Bind geometry, material conditions, thermal and mechanical boundaries, process sequence, observables and comparator conditions. Keep original observations separate from reconstructed assumptions and counterfactual variants. For every non-missing literature-derived or reconstruction-derived EPR envelope, record an inverse map from the exact candidate/global EPR path to reviewed direct, converted, derived, fitted or assumed provenance and all dependency parameters/source assets; the EPR's single-reference provenance envelope remains unchanged. Before execution, distinguish that complete value mapping from the expected consumed paths for each selected candidate and requested scenario (or their frozen derivation rule). Classify structural paths and declared missing/conflicting inputs separately. Bind each explicit Plan override to its frozen Plan value and reviewed source, transformation or stated hypothetical assumption; it must not inherit a paper's authority for a different number.
4. **Create the one authoritative pre-execution freeze.** For each computational validation activity, an immutable pre-execution LVS revision contains the **LECF validation-freeze manifest**. It binds exact LIT/LEM/LPE/LCR revisions and identities; source-asset versions and byte identities; EPR `compiled_content_sha256` and exact file SHA-256; the complete canonical Evaluation Plan snapshot and its `evaluation_plan_sha256`; the resolved complete canonical Model Manifest snapshot and independently recomputed `model_manifest_sha256`; M16A unit-registry and deterministic runtime-policy versions; expected consumed paths per selected candidate/scenario or a derivation algorithm and version bound to these frozen EPR/Plan/Manifest/version inputs; target sample/dataset partitions and roles; and executor, custodian, known prior exposure, intended visibility and reveal controls. It also records targets, metrics, fitting/model-selection history, uncertainty treatment, controls, exclusions and invalid-run rules. Numerical tolerances require a case-specific evidence basis and independent review; this proposal supplies none. The pilot cannot defer Manifest selection to execution.
5. **Check before reveal, then execute.** Before any executor receives outcomes, an independent reviewer resolves every frozen reference, checks source/file identities, reconstructs the EPR identities, reconstructs the complete Plan and Manifest through the existing M16A canonical identity implementation, recomputes both hashes, and checks expected scenario paths, target partitions, input roles and exposure declarations. The final reviewer attestation is recorded outside the immutable freeze content, binds its exact LVS Git/file identity, and must pass before execution/reveal. The Plan snapshot is LECF registration evidence inside the LVS freeze, not a third primary M16A Plan sidecar and not an `engineering/plans` artifact. The existing kernel then executes only a valid bound Plan. The eventual EER's embedded Plan and Manifest must match their frozen snapshots canonically and by hash; a mismatch is not a predeclared run. EER records all planned, failed and non-evaluable scenarios.
6. **Bind outcomes without changing the reconstruction.** A later LVS revision references the unchanged frozen LCR revision and exact validation-freeze identity, then binds actual execution and reveal facts, EER file/content identities, embedded Plan and Manifest hashes, `evaluation_input_sha256`, outputs, applicability findings and each scenario's actual `consumed_input_paths`. Compare each actual path set with the frozen expectation for that candidate/scenario, including OAT-only paths; investigate unexplained extra, missing or ambiguous paths. An expected OAT difference, structural path or declared missing/conflicting input is not by itself a provenance failure. Every actual non-missing literature/reconstruction input, including a Plan override value, still needs its reviewed mapping and basis. For `blocked`, `invalid` or `not_applicable`, retain paths and findings without claiming a source-to-numerical-output result. Project only eligible prediction outputs. Compare with eligible evidence and record residuals and possible parameter/model/boundary/measurement causes without attributing cause from residual alone. The independent reviewer checks scope, lineage, exposure and all adverse findings before considering candidacy. Rule/model/reconstruction changes require a new LCR revision, new freeze and new evaluation.
...
The manual pilot uses Markdown working records. The validation-freeze manifest binds each dependency's exact Git commit/path/blob identity and exact file SHA-256; the final reviewer attestation is outside that immutable manifest and names the authoritative LVS revision's own exact Git/file identity so the manifest does not recursively self-hash. These records do not pretend to possess the future canonical JSON `record_content_sha256`. Required facts not yet knowable at registration or before execution remain explicitly pending with reason, responsible owner and resolution gate. Pending fields cannot be treated as reviewed, frozen or satisfied, and freeze-required fields must be resolved before execution. Actual execution and reveal facts remain pending until they occur and are recorded in the later LVS revision.
