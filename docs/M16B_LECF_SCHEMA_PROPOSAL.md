...
All six records require the fields below. “Required” means the field must be present; a null value is allowed only where stated and must carry a reason. Empty strings cannot disguise missing information.
...
| `record_content_sha256` | Lowercase SHA-256 for a future canonical record excluding only this self-hash. Includes governance metadata; distinct from M16A reproducibility identities. Manual Markdown instead uses exact Git blob/file identity and must not claim this JSON identity. |
...
| `source_version` | Publisher/preprint version label, publication/revision date, `source_family_id`, related versions and their relationships. Do not merge preprint, accepted manuscript, final article and supplement identities. |
...
| `dataset_families` | Known experiment/dataset-family IDs, related reports and reuse evidence, or `unknown` plus investigation action. Different DOIs are not independence evidence. |
...
A relation requires `relation_id`, fully qualified `from_ref`/`to_ref`, `relation_type`, `scope`, and `basis`. Proposed types are `transcribed_from`, `derived_from`, `supports`, `contradicts`, `background_for`, `conflicts_with`, `unknown_relation`, `same_dataset_as`, and `supersedes`. Preserve all conflicting assertions; do not average them or elect a winner silently. Cross-record derivation cycles reject the mapping.
...
| `sample` and `dataset_ref` | Paper sample/series label, sanitized local sample alias, count or missing reason, specimen state and dataset-family identity. Never combine different specimens or process stages silently. |
...
| `epr_mapping` | Parameter IDs -> exact EPR field paths including candidate paths, transformation, provenance strategy, intended envelope status and review refs. Binding includes persisted EPR reference, `compiled_content_sha256`, file hash and version when available; otherwise null/pending. |
| `evaluation_bindings` | Exact EER file/content identities, embedded Plan hash, Manifest hash, candidate/output identities and pointers to actual applicability findings. Empty before execution. No duplicate editable Plan or EER. |
...
| Required field | Meaning |
| --- | --- |
| `scope` | Exact reconstruction/case revisions, intended use, observable/reference plane, operating envelope and limits. |
...
| `exposure_and_freeze` | Executor/custodian roles, prior outcome access, protocol/input/baseline identities, freeze/reveal events, and retrospective/blind classification with evidence. Null fields need reasons; no invented blind baseline. |
| `evaluation_refs`, `measurement_refs`, `comparison_refs` | Exact EPR/EER/Plan hash, EVD/MSR/PRL references and target bindings. Record empty lists for non-computational checks. |
...
| Lateral spreading or unequal direct-convection area is material | Hold/`not_applicable` for strict 1D; retain case for another reviewed method. |
...
Independent architecture review must confirm ID scope, the six-record burden, source-version/dataset-family deduplication, rights handling and certification vocabulary before a manual pilot. Executable schema/serialization design, safe writers, review-package extensions and enforcement of these gates require a separately authorized implementation task. Existing validators do not enforce the proposed LECF lifecycle or correction propagation.
