...
| `record_content_sha256` | Lowercase SHA-256 for a future canonical JSON record excluding only this self-hash. Includes governance metadata; distinct from M16A reproducibility identities. A manual Markdown record does not populate or claim this future identity. For each bound manual dependency, the LECF validation-freeze manifest records its exact Git commit, repository-relative path, Git blob OID with algorithm, and exact file SHA-256. The final reviewer attestation is outside the immutable freeze content and names the authoritative LVS revision's exact Git/file identity, avoiding a recursive hash. |
...
| `intended_input_paths` | Expected `consumed_input_paths` per selected candidate and requested core/OAT scenario, or a frozen derivation algorithm and version using exact EPR, Plan, Manifest/input-binding and runtime-policy identities. Classify each expected path as structural, non-missing value, or declared missing/conflicting input; link a non-missing literature/reconstruction envelope to its `epr_mapping`, a structural path to its reviewed reconstruction basis, or another source as applicable. No single union inventory substitutes for the scenario sets. Freeze the algorithm/version or sets before execution. |
| `plan_override_mapping` | For each explicit sweep/OAT point, bind candidate ID, scenario/point coordinate, exact field path and frozen Plan value to its reviewed source, transformation/dependencies or stated hypothetical assumption. The override uses the same consumed path but replaces that scenario's numeric value; it cannot silently claim the base EPR value's literature provenance. Pending review makes the intended use ineligible for freeze. |
...
| `pre_execution_freeze` | Null for a non-computational check. For the authoritative freeze, contains the LECF validation-freeze manifest: exact LIT/LEM/LPE/LCR revisions and manual/future content identities; exact source-asset versions and byte hashes; EPR reference, `compiled_content_sha256` and exact file SHA-256; complete canonical Evaluation Plan snapshot and recomputed `evaluation_plan_sha256`; resolved complete canonical Model Manifest snapshot and independently recomputed `model_manifest_sha256` (including its explicit `implementation_git_commit`, which may be null); M16A unit-registry and deterministic runtime-policy versions; expected per-candidate/scenario consumed paths or their frozen derivation rule bound to those exact inputs; target sample/dataset partitions and roles; and pre-freeze source/reconstruction review references. The pilot cannot defer Manifest selection to execution. A post-execution revision references this exact freeze identity without changing it. |
| `exposure_and_freeze` | Executor/custodian roles, known prior outcome access, intended visibility/reveal controls, and retrospective/blind classification with evidence. All freeze-required facts are resolved in the authoritative pre-execution revision; actual execution and reveal facts remain pending until they occur. No invented blind baseline. |
| `execution_bindings` | Empty in the authoritative pre-execution freeze. A post-execution LVS revision records actual execution/reveal events, exact EER file/content identities, embedded Plan and Manifest hashes, `evaluation_input_sha256`, candidate/output identities, pointers to actual applicability findings and each scenario's actual `consumed_input_paths`; it also names the unchanged frozen LCR revision and exact freeze identity. No duplicate editable Plan or EER. |
...
The one authoritative pre-execution LVS revision is immutable after its exact record/file identity is registered. Before outcome reveal, an independent reviewer resolves and checks every frozen reference and source-asset byte identity; reconstructs EPR content and file identities; reconstructs the complete Plan and resolved Manifest with the existing M16A canonical identity implementation and recomputes both hashes; and checks the scenario-path expectation or frozen derivation rule, target partitions/roles, custodian/executor separation and exposure declarations. The final reviewer attestation is a later event outside the frozen content, identifies that exact LVS revision and Git/file identity, and must pass before execution. A missing, pending or mismatched freeze field blocks execution under this protocol.

The complete Plan snapshot is registration evidence embedded in the LECF freeze, not a standalone primary M16A Plan sidecar and never an `engineering/plans` artifact. The eventual EER must embed a Plan and Model Manifest canonically identical to their frozen snapshots and carry the same `evaluation_plan_sha256` and `model_manifest_sha256`; its unit-registry and runtime-policy versions must also match the freeze. A mismatch means the execution was not the predeclared run. The post-execution LVS revision preserves the freeze and LCR references while adding actual execution bindings; it never edits or substitutes them.

After execution, compare each scenario's actual `consumed_input_paths` with its own frozen expected set or the set recomputed from the frozen derivation rule. Investigate unexplained extra, missing or ambiguous paths; an expected OAT-only path, structural discriminator or declared missing/conflicting input is not by itself a provenance failure. Independently check that every actual non-missing literature-derived or reconstruction-derived envelope resolves to exactly one reviewed `epr_mapping` with all dependencies; a substituted Plan value must also resolve to its reviewed `plan_override_mapping`. An unexplained path or unmapped non-missing value prevents a complete traceability claim for the affected scenario. Record the actual disposition and numerical-result presence separately: `blocked`, `invalid` and `not_applicable` scenarios retain paths and findings but have no source-to-numerical-output claim, even if their expected path sets match.
...
| Actual scenario contains an unexplained consumed path or an unmapped non-missing literature/reconstruction value or Plan override | Investigate and withhold that scenario's complete traceability claim; a correction to frozen mapping, Plan or reconstruction requires a new reviewed freeze/evaluation cycle. Expected OAT-only paths and declared missing/conflicting inputs are not provenance failures. |
