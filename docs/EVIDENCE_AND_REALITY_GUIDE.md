...
A public-safe Gold Case can expose a deterministic screening gap after a frozen baseline is compared with later evidence. M14 thermomechanical triage consumes only the available case context and optional sidecar status to request missing evidence; it does not learn numerical thresholds, modify rules automatically, or treat one literature result as general rule-family validation.

An artifact-separated benchmark must commit and hash its canonical case and exported baseline before adding source Evidence Objects, Measurement References, or Prediction-Reality Records. The later assessment must report the actual baseline output, including any missed rule, false positive, scope mismatch, or Decision Board coherence defect, without rewriting the historical artifacts. M15A is outcome-value-withheld but retrospective and not agent-blind. A stronger M15B benchmark should use separate knowledge-isolated execution, predeclared scoring, and an independent negative or boundary control. Draft source-documented measurements remain pending human review and do not provide automatic calibration or engineering-memory promotion.
